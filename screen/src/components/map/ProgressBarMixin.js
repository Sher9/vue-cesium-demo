...
export const ProgressBarMixin = {
    ...
    computed: {
        ...
        lineProgressHeight() {
            return px(
                this.defaultOptions.layout.height -
                    this.defaultOptions.layout.progressPadding
            )
        },
        batteryProgress() {
            return {
                ...
                width: px(
                    this.value *
                        ((this.defaultOptions.layout.width -
                            this.defaultOptions.layout.progressPadding) /
                            100)
                ),
            }
        ...
        batteryStyleSvgFrame() {
            return {
                ...
                width: px(
                    this.defaultOptions.layout.width +
                        this.defaultOptions.layout.width / 16
                ),
            }
        },
        ...
        horizontalTextAlignP() {
            if (this.defaultOptions.text.dynamicPosition) {
                ...
                if (this.value === 0 && this.line) {
                    return (
                        this.defaultOptions.layout.horizontalTextAlign +
                        this.defaultOptions.layout.zeroOffset +
                        '%'
                    )
                } else {
        ...
        textStyle() {
            return {
                ...
                textShadow: this.defaultOptions.text.shadowEnable
                    ? '1px 1px 1px ' + this.defaultOptions.text.shadowColor
                    : 'none',
            }
        },
        textStyleCircle() {
    ...
    methods: {
        ...
        updateValue(val) {
            ...
            let heightPrecent = val / 100
            if (this.cylinder) {
                this.rectHeight = 40 * heightPrecent
                this.rectY = invertedVal * 0.4 + 20
                this.topCy = -invertedVal * -0.4 + 20
                ...
                this.strokeCircleOffset = this.defaultOptions.progress.inverted
                    ? this.strokeCircle * (val / 100) + this.strokeCircle
                    : this.strokeCircle * ((100 - val) / 100)
            }
        },
        // Used for cylinder
